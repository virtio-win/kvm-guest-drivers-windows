...
printf '  fresh connect   : %s\n' "$([ "$probe_ok" -eq 1 ] && echo ok || echo FAIL)"

if [ "$save_rc" -eq 0 ] && [ "$restore_rc" -eq 0 ] \
   && [ "$guest_up" -eq 1 ] && [ "$driver_ok" -eq 1 ] && [ "$probe_ok" -eq 1 ]; then
    info "=== PASS ==="
    exit 0
else
    err "=== FAIL (save=$save_rc restore=$restore_rc up=$guest_up driver=$srv_state probe=$probe_ok) ==="
    exit 1
fi
