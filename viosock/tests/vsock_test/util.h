...
/* All known vsock transports. */
#define KNOWN_TRANSPORTS(x)                                                                                            \
    x(LOOPBACK, "loopback") x(VIRTIO, "virtio") x(VHOST, "vhost") x(VMCI, "vmci") x(HYPERV, "hvs")

enum transport
{
    TRANSPORT_COUNTER_BASE = __COUNTER__ + 1,
#define x(name, symbol) TRANSPORT_##name = BIT(__COUNTER__ - TRANSPORT_COUNTER_BASE),
    KNOWN_TRANSPORTS(x) TRANSPORT_NUM = __COUNTER__ - TRANSPORT_COUNTER_BASE,
#undef x
};
...
static_assert(ARRAY_SIZE(transport_ksyms) == TRANSPORT_NUM, "transport_ksyms size mismatch");
static_assert(BITS_PER_TYPE(int) >= TRANSPORT_NUM, "int too narrow for transport bitmask");
