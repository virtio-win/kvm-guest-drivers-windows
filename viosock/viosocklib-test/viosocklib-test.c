// viosocklib-test.c : This file contains the 'main' function. Program execution
...
\tdwProviderReserved: %u\n\n"),
      pProtocolInfo->szProtocol, pProtocolInfo->dwServiceFlags1,
      pProtocolInfo->dwServiceFlags2, pProtocolInfo->dwServiceFlags3,
      pProtocolInfo->dwServiceFlags4, pProtocolInfo->dwProviderFlags,
      pProtocolInfo->ProviderId.Data1, pProtocolInfo->ProviderId.Data2,
      pProtocolInfo->ProviderId.Data3, pProtocolInfo->ProviderId.Data4[0],
      pProtocolInfo->ProviderId.Data4[1], pProtocolInfo->ProviderId.Data4[2],
      pProtocolInfo->ProviderId.Data4[3], pProtocolInfo->ProviderId.Data4[4],
      pProtocolInfo->ProviderId.Data4[5], pProtocolInfo->ProviderId.Data4[6],
      pProtocolInfo->ProviderId.Data4[7], pProtocolInfo->dwCatalogEntryId,
      pProtocolInfo->ProtocolChain.ChainLen, pProtocolInfo->iVersion,
      pProtocolInfo->iAddressFamily, pProtocolInfo->iMaxSockAddr,
      pProtocolInfo->iMinSockAddr, pProtocolInfo->iSocketType,
      pProtocolInfo->iProtocol, pProtocolInfo->iProtocolMaxOffset,
      pProtocolInfo->iNetworkByteOrder, pProtocolInfo->iSecurityScheme,
      pProtocolInfo->dwMessageSize, pProtocolInfo->dwProviderReserved);
}
...
BOOL EnumProtocols32() {
  ...
  if (iRes == ERROR_SUCCESS || iErrno != WSAENOBUFS) {
    ...
    if (pProtocolList) {
      int iProtos =
          WSCEnumProtocols32(NULL, pProtocolList, &lpdwBufferLength, &iErrno);
...
BOOL EnumProtocols() {
  ...
  if (iRes == ERROR_SUCCESS || iErrno != WSAENOBUFS) {
    ...
    if (pProtocolList) {
      int iProtos =
          WSCEnumProtocols(NULL, pProtocolList, &lpdwBufferLength, &iErrno);
      if (iProtos == SOCKET_ERROR) {
...
BOOL AddBufferToFile(PTCHAR sFileName, PVOID Buffer, ULONG BufferLen) {
  BOOL bRes = FALSE;
  HANDLE hFile = CreateFile(sFileName, GENERIC_WRITE, 0, NULL, OPEN_ALWAYS,
...
int __cdecl _tmain(int argc, _TCHAR *argv[]) {
  ...
  case _T('i'):
    if (!InstallProtocol()) {
      switch (GetLastError()) {
      case WSAEACCES:
        _tprintf(_T("Access denied. Protocol not registered, administrative ")
                 _T("rights required.\n"));
        break;
      case ERROR_NOT_SUPPORTED:
        _tprintf(_T("Protocol not registered, use x64 app to register both ")
                 _T("x86 and x64 protocol libraries\n"));
        break;
      default:
  ...
  case _T('d'):
    if (!DeinstallProtocol()) {
      switch (GetLastError()) {
      case WSAEACCES:
        _tprintf(_T("Access denied. Protocol not unregistered, administrative ")
                 _T("rights required.\n"));
        break;
      case ERROR_NOT_SUPPORTED:
        _tprintf(_T("Protocol not unregistered, use x64 app to unregister ")
                 _T("both x86 and x64 protocol libraries\n"));
        break;
      default:
