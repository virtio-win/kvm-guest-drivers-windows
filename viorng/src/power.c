                                          ...
                                          USHORT Vector)
{
	...
	if (size)
	{
		...
		if (p)
		{
			vq = VirtIODevicePrepareQueue(VirtDevice, Index, MmGetPhysicalAddress(p),
                p, size, p, FALSE);
		}
	}
                                 ...
                                 IN WDF_POWER_DEVICE_STATE PreviousState)
{
    ...
    vector = info.MessageSignaled ? 1 : VIRTIO_MSI_NO_VECTOR;
                                ...
                                IN WDF_POWER_DEVICE_STATE TargetState)
{
    ...
    context->VirtQueue->vq_ops->shutdown(context->VirtQueue);
