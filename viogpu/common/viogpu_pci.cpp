...
void *mem_alloc_contiguous_pages(void *context, size_t size) {
  ...
  if (ptr) {
    ...
    DbgPrint(TRACE_LEVEL_FATAL,
             ("Ran out of memory in alloc_pages_exact(%Id)\n", size));
  }
  return ptr;
}
...
static int PCIReadConfig(VioGpuAdapter *pdev, int where, void *buffer,
                         size_t length) {
  PAGED_CODE();
  ...
  Status = pDxgkInterface->DxgkCbReadDeviceSpace(
      pDxgkInterface->DeviceHandle, DXGK_WHICHSPACE_CONFIG, buffer, where,
      (ULONG)length, &BytesRead);
...
VirtIOSystemOps VioGpuSystemOps = {
    ReadVirtIODeviceByte,       ReadVirtIODeviceWord,
    ReadVirtIODeviceRegister,   WriteVirtIODeviceByte,
    WriteVirtIODeviceWord,      WriteVirtIODeviceRegister,
    mem_alloc_contiguous_pages, mem_free_contiguous_pages,
    mem_get_physical_address,   mem_alloc_nonpaged_block,
    mem_free_nonpaged_block,    pci_read_config_byte,
    pci_read_config_word,       pci_read_config_dword,
    pci_get_resource_len,       pci_map_address_range,
    vdev_get_msix_vector,       vdev_sleep,
};
    ...
    if (m_bPortSpace) {
      if (m_bIoMapped) {
        Status = pDxgkInterface->DxgkCbMapMemory(pDxgkInterface->DeviceHandle,
                                                 m_BasePA, m_uSize, TRUE, FALSE,
                                                 MmNonCached, &m_BaseVA);
...
bool CPciResources::Init(PDXGKRNL_INTERFACE pDxgkInterface,
  ...
  DbgPrint(TRACE_LEVEL_VERBOSE, ("---> %s\n", __FUNCTION__));
  Status = m_pDxgkInterface->DxgkCbReadDeviceSpace(
      m_pDxgkInterface->DeviceHandle, DXGK_WHICHSPACE_CONFIG, &pci_config, 0,
      sizeof(pci_config), &BytesRead);
  ...
  if (BytesRead != sizeof(pci_config)) {
    DbgPrint(TRACE_LEVEL_ERROR,
             ("[%s] could not read PCI config space\n", __FUNCTION__));
    return false;
  }
  DbgPrint(TRACE_LEVEL_VERBOSE,
    ...
    for (ULONG j = 0; j < pFullResDescriptor->PartialResourceList.Count; ++j) {
      ...
      case CmResourceTypeInterrupt: {
        ...
        if (IsMSIEnabled()) {
          DbgPrint(
              TRACE_LEVEL_FATAL,
              ("Found MSI Interrupt vector %d, level %d, affinity 0x%X, flags "
               "%X\n",
...
PAGED_CODE_SEG_BEGIN
NTSTATUS
MapFrameBuffer(_In_ PHYSICAL_ADDRESS PhysicalAddress, _In_ ULONG Length,
               _Outptr_result_bytebuffer_(Length) VOID **VirtualAddress) {
  PAGED_CODE();

  if ((PhysicalAddress.QuadPart == (ULONGLONG)0) || (Length == 0) ||
      (VirtualAddress == NULL)) {
    DbgPrint(TRACE_LEVEL_ERROR,
             ("One of PhysicalAddress.QuadPart (0x%I64x), Length (%lu), "
  ...
  *VirtualAddress = MmMapIoSpace(PhysicalAddress, Length, MmWriteCombined);
  ...
  DbgPrint(TRACE_LEVEL_FATAL,
           ("%s PhysicalAddress.QuadPart (0x%I64x), Length (%lu), "
            "VirtualAddress (%p)\n",
...
NTSTATUS
UnmapFrameBuffer(_In_reads_bytes_(Length) VOID *VirtualAddress,
                 _In_ ULONG Length) {
  PAGED_CODE();
  ...
  if ((VirtualAddress == NULL) && (Length == 0)) {
    ...
    DbgPrint(TRACE_LEVEL_ERROR,
             ("Only one of Length (%lu), VirtualAddress (%p) is NULL or 0",
