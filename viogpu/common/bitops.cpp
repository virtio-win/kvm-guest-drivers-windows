...
VOID GetPitches(_In_ CONST BLT_INFO *pBltInfo, _Out_ LONG *pPixelPitch,
  ...
  default:
    VioGpuDbgBreak();
    VIOGPU_LOG_ASSERTION1("Invalid rotation (0x%I64x) specified",
                          pBltInfo->Rotation);
    *pPixelPitch = 0;
    *pRowPitch = 0;
    return;
...
BYTE *GetRowStart(_In_ CONST BLT_INFO *pBltInfo, _In_ CONST RECT *pRect) {
  ...
  switch (pBltInfo->Rotation) {
  case D3DKMDT_VPPR_IDENTITY:
    pRet = ((BYTE *)pBltInfo->pBits + (ULONGLONG)OffTop * pBltInfo->Pitch +
...
VOID CopyBitsGeneric(_In_ BLT_INFO *pDst, _In_ CONST BLT_INFO *pSrc,
...
VOID CopyBits32_32(_In_ BLT_INFO *pDst, _In_ CONST BLT_INFO *pSrc,
                   _In_ CONST RECT *pRect) {
...
VOID BltBits(_In_ BLT_INFO *pDst, _In_ CONST BLT_INFO *pSrc,
             _In_ CONST RECT *pRect) {
  DbgPrint(TRACE_LEVEL_VERBOSE, ("---> %s\n", __FUNCTION__));
  __try {
...
#pragma prefast(                                                               \
  ...
  __except (EXCEPTION_EXECUTE_HANDLER) {
    DbgPrint(TRACE_LEVEL_ERROR, ("Either dst (0x%p) or src (0x%p) bits "
                                 "encountered exception during access.\n",
                                 pDst->pBits, pSrc->pBits));
  }
}
