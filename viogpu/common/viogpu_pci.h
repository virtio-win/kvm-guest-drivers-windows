...
public:
  CPciBar(PHYSICAL_ADDRESS BasePA, ULONG uSize, bool bPortSpace, bool bIoMapped)
      : m_BasePA(BasePA), m_uSize(uSize), m_BaseVA(nullptr),
        m_bPortSpace(bPortSpace), m_bIoMapped(bIoMapped) {
    ASSERT(!m_bPortSpace || m_BasePA.HighPart == 0);
  }

  CPciBar() : CPciBar(PHYSICAL_ADDRESS(), 0, false, true) {}

  ~CPciBar() { ASSERT(m_BaseVA == nullptr); }

  ULONG GetSize() { return m_uSize; }

  bool IsPortSpace() { return m_bPortSpace; }

  PHYSICAL_ADDRESS GetPA() { return m_BasePA; }
...
public:
  ...
  USHORT GetInterruptFlags() { return m_InterruptFlags; }
