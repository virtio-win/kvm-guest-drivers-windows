...
#define DbgPrint(Level, MSG)                                                   \
  if (Level <= nDebugLevel)                                                    \
    VirtioDebugPrintProc(DbgExpandArguments MSG);
#define VioGpuDbgBreak()                                                       \
  if (KD_DEBUGGER_ENABLED && !KD_DEBUGGER_NOT_PRESENT && bBreakAlways)         \
    DbgBreakPoint();

#define WPP_INIT_TRACING(driver, regpath)                                      \
  InitializeDebugPrints(driver, regpath);
...
#define WPP_CONTROL_GUIDS                                                      \
  WPP_DEFINE_CONTROL_GUID(VioGpuDodTraceGuid,                                  \
