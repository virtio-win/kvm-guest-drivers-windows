...
PipeServer::PipeServer(const std::wstring &sName)
    : m_sPipeName(sName), m_hPipe(INVALID_HANDLE_VALUE), m_hThread(NULL),
      m_buffer(NULL) {
  PrintMessage(L"%ws\n", __FUNCTIONW__);
...
void PipeServer::SetData(std::wstring &sData) {
  ...
  wcsncpy_s(&m_buffer[0], DATA_BUFFER_LENGTH, sData.c_str(),
            __min(DATA_BUFFER_LENGTH, sData.size()));
}
...
bool PipeServer::Init() {
  ...
  m_hThread = ::CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ServerThread,
                             (LPVOID)this, 0, NULL);
...
bool PipeClient::Init() {
  ...
  m_hThread = ::CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ClientThread,
                             (LPVOID)this, 0, NULL);
...
void PipeClient::ConnectToServer() {
  ...
  m_hPipe = ::CreateFile(m_sPipeName.c_str(), GENERIC_READ | GENERIC_WRITE, 0,
...
bool PipeClient::Read() {
  ...
  do {
    bFinishedRead =
        ::ReadFile(m_hPipe, &m_buffer[read], DATA_BUFFER_SIZE, &drBytes, NULL);
