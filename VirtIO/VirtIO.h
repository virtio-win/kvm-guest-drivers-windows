...
typedef void *(*proc_virtqueue_get_buf)(struct virtqueue *vq,
                                        unsigned int *len);
...
/* Represents one virtqueue; only data pointed to by the vring structure is
 * exposed to the host */
struct virtqueue {
  VirtIODevice *vdev;
  unsigned int index;
...
static inline int virtqueue_add_buf(struct virtqueue *vq,
                                    ...
                                    void *opaque, void *va_indirect,
                                    ULONGLONG phys_indirect) {
  return vq->add_buf(vq, sg, out_num, in_num, opaque, va_indirect,
                     phys_indirect);
}
