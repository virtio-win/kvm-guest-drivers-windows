...
VOID EvtControlDeviceIoControl(WDFQUEUE Queue,
                               WDFREQUEST Request,
...
NTSTATUS EvtDevicePrepareHardware(WDFDEVICE Device, WDFCMRESLIST ResourcesRaw, WDFCMRESLIST ResourcesTranslated)
{
    ...
    NTSTATUS status = WdfIoTargetSendIoctlSynchronously(target,
                                                        NULL,
    ...
    // Use Non-Paged pool because we are doing hardware I/O (NX for security; the
    // project opts in via POOL_NX_OPTIN). ExAllocatePoolWithTag is used instead of
    // ExAllocatePool2 to keep the Windows 10 base target compatible with the rest
    // of the tree.
    PUCHAR buffer = (PUCHAR)ExAllocatePoolWithTag(NonPagedPoolNx, totalBufferSize, 'IPDS');
    if (!buffer)
    {
    ...
    status = WdfIoTargetSendIoctlSynchronously(target,
                                               NULL,
        ...
        if (pSpt->ScsiStatus == SCSISTAT_GOOD)
        {
            ...
            if (pVpd->PageCode == 0xB0)
            {
                ...
                if (maxTransferBlocks > 0)
                {
                    ...
                    // Compute in 64-bit to avoid 32-bit overflow when the hardware reports
                    // a very large maximum transfer length (blocks * sectorSize can exceed 4GB).
                    size_t maxBytes = (size_t)maxTransferBlocks * devCtx->SectorSize;
                    DbgPrint("[SCSI INQUIRY] HW Max Transfer: %u blocks (%Iu bytes)\n", maxTransferBlocks, maxBytes);
...
VOID EvtChildRequestCompleted(WDFREQUEST ChildRequest,
                              WDFIOTARGET Target,
                              ...
                              WDFCONTEXT Context)
{
    ...
    // If this chunk failed, record the failure. Protect with a spinlock since chunks complete concurrently on different
    // CPU cores.
    ...
    if (InterlockedDecrement(&parentCtx->OutstandingChildren) == 0)
    {
        ...
        size_t bytesCompleted = NT_SUCCESS(finalStatus) ? (parentParams.Type == WdfRequestTypeRead ? parentParams.Parameters.Read.Length
                                                                                                   : parentParams.Parameters.Write.Length)
                                                        : 0;
...
VOID EvtIoRead(WDFQUEUE Queue, WDFREQUEST Request, size_t Length)
{
    ...
    while (memoryOffset < Length)
    {
        ...
        if (chunkSize > activeSafeSize)
        {
            chunkSize = activeSafeSize; // Clamp size to hardware max
        }
