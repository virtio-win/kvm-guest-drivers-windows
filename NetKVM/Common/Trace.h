...
#define WPP_CONTROL_GUIDS                                                                                              \
    WPP_DEFINE_CONTROL_GUID(NetKVM, (5666D67E, 281E, 43ED, 8B8D, 4347080198AA),                                        \
                            WPP_DEFINE_BIT(TRACE_DRIVER) /* bit  0 = 0x00000000 */                                     \
    )

#define WPP_Flags_LEVEL_LOGGER(Flags, level) WPP_LEVEL_LOGGER(Flags)
#define WPP_Flags_LEVEL_ENABLED(Flags, level) (level <= virtioDebugLevel)

#define WPP_Flags_LEVEL_STATUS_LOGGER(Flags, level, status) WPP_LEVEL_LOGGER(Flags)
#define WPP_Flags_LEVEL_STATUS_ENABLED(Flags, level, status) (level <= virtioDebugLevel)
