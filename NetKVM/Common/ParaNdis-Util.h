...
void ParaNdis_PrintTable(int DebugPrintLevel, TTable table, size_t Size, LPCSTR Format, AccessorsFuncs... Accessors)
{
    ...
    while (i < Size)
    {
        ...
        {
            Res = RtlStringCbPrintfExA(LinePos, ColumnWidth, &End, NULL, STRSAFE_FILL_ON_FAILURE | ' ', FullFormat, i,
                                       Accessors(table + i)...);
...
static inline void ParaNdis_CompleteNBLChainWithStatus(NDIS_HANDLE MiniportHandle, PNET_BUFFER_LIST NBL,
                                                       NDIS_STATUS Status, ULONG Flags = 0)
{
