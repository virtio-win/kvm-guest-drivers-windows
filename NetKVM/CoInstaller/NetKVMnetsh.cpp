...
tstring _NetKVMQueryDeviceString(HDEVINFO hDeviceSet, PSP_DEVINFO_DATA DeviceInfoData, DWORD dwPropertyID)
{
    ...
    while (!SetupDiGetDeviceRegistryProperty(hDeviceSet, DeviceInfoData, dwPropertyID, &dwDataType,
                                             (PBYTE)szDeviceString, dwSize, &dwSize))
    {
        DWORD dwErr = GetLastError();
...
DWORD _NetKVMQueryDeviceDWORD(HDEVINFO hDeviceSet, PSP_DEVINFO_DATA DeviceInfoData, DWORD dwPropertyID)
{
    ...
    if (!SetupDiGetDeviceRegistryProperty(hDeviceSet, DeviceInfoData, dwPropertyID, &dwDataType, (PBYTE)&dwDeviceDword,
                                          sizeof(dwDeviceDword), NULL))
    {
        return 0;
    }
...
static bool _NetKVMGetKnownDevices(GUID *pguidDevClassPtr, vector<_NetKVMDeviceInfo> &Devices, HDEVINFO *phDeviceInfo)
{
    *phDeviceInfo = SetupDiGetClassDevsEx(pguidDevClassPtr, NULL, NULL, DIGCF_PRESENT, NULL, NULL, NULL);
    ...
    for (DWORD dwDevIndex = 0; SetupDiEnumDeviceInfo(*phDeviceInfo, dwDevIndex, &CurrDeviceInfo); dwDevIndex++)
    {
        ...
        CONFIGRET err = CM_Get_Device_ID_Ex(CurrDeviceInfo.DevInst, szDeviceId, MAX_DEVICE_ID_LEN, 0,
...
static DWORD _NetKVMRestartDevice(DWORD dwIndex)
{
    ...
    if (!SetupDiSetClassInstallParams(g_hDeviceInfoList, &g_DevicesOfInterest[dwIndex].DevInfoData,
    ...
    if (!SetupDiGetDeviceInstallParams(g_hDeviceInfoList, &g_DevicesOfInterest[dwIndex].DevInfoData,
                                       &DeviceInstallParams))
    {
...
static bool _NetKVMQueryDetailedParamInfo(DWORD dwDeviceIndex, const tstring &strParamName,
                                          neTKVMRegParamType &ParamType, neTKVMRegParamExInfoList &ParamInfo)
{
    neTKVMRegAccess DeviceRegKey(HKEY_LOCAL_MACHINE, g_DevicesOfInterest[dwDeviceIndex].strRegPathName.c_str());
...
//
DWORD WINAPI _NetKVMShowDevicesCmdHandler(__in PWCHAR /*pwszMachine*/, __in PWCHAR * /*ppwcArguments*/,
                                          __in DWORD /*dwCurrentIndex*/, __in DWORD /*dwArgCount*/,
                                          __in DWORD /*dwFlags*/, __in PVOID /*pvData*/, __out BOOL *pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
    ...
    try
    {
        ...
        for (vector<_NetKVMDeviceInfo>::const_iterator it = g_DevicesOfInterest.begin();
             it != g_DevicesOfInterest.end(); ++it, ++i)
        {
...
//
DWORD WINAPI _NetKVMShowParamInfoCmdHandler(__in PWCHAR /*pwszMachine*/, __in PWCHAR *ppwcArguments,
                                            __in DWORD dwCurrentIndex, __in DWORD dwArgCount, __in DWORD /*dwFlags*/,
                                            __in PVOID /*pvData*/, __out BOOL *pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
    ...
    try
    {
        ...
        DWORD dwPreprocessResult =
            PreprocessCommand(NULL, ppwcArguments, dwCurrentIndex, dwArgCount, TagsList, ARRAY_SIZE(TagsList),
                              ARRAY_SIZE(TagsList), ARRAY_SIZE(TagsList), pdwTagMatchResults.get());
        ...
        case NO_ERROR: {
            ...
            if (__NetKVMConvertDeviceIndex(ppwcArguments[dwCurrentIndex + pdwTagMatchResults.get()[0]], &dwIndex))
            {
                ...
                for (neTKVMRegParamExInfoList::const_iterator it = ParamInfo.begin(); it != ParamInfo.end(); ++it)
                {
                    switch (it->first)
                    {
...
//
DWORD WINAPI _NetKVMGetParamCmdHandler(__in PWCHAR /*pwszMachine*/, __in PWCHAR *ppwcArguments,
                                       __in DWORD dwCurrentIndex, __in DWORD dwArgCount, __in DWORD /*dwFlags*/,
                                       __in PVOID /*pvData*/, __out BOOL *pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
...
//
DWORD WINAPI _NetKVMSetParamCmdHandler(__in PWCHAR /*pwszMachine*/, __in PWCHAR *ppwcArguments,
                                       __in DWORD dwCurrentIndex, __in DWORD dwArgCount, __in DWORD /*dwFlags*/,
                                       __in PVOID /*pvData*/, __out BOOL *pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
...
//      show parameters 2
//
DWORD WINAPI _NetKVMShowParamsCmdHandler(__in PWCHAR /*pwszMachine*/, __in PWCHAR *ppwcArguments,
                                         __in DWORD dwCurrentIndex, __in DWORD dwArgCount, __in DWORD /*dwFlags*/,
                                         __in PVOID /*pvData*/, __out BOOL *pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
...
//      restart 2
//
DWORD WINAPI _NetKVMRestartDeviceCmdHandler(__in PWCHAR /*pwszMachine*/, __in PWCHAR *ppwcArguments,
                                            __in DWORD dwCurrentIndex, __in DWORD dwArgCount, __in DWORD /*dwFlags*/,
                                            __in PVOID /*pvData*/, __out BOOL *pbDone)
{
    *pbDone = FALSE; /* Just to make static analyzer happy */
...
CMD_ENTRY g_ShowCmdTable[] = {CREATE_CMD_ENTRY_EX(NETKVM_SHOW_DEVICES, (PFN_HANDLE_CMD)_NetKVMShowDevicesCmdHandler,
                                                  CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL),
                              CREATE_CMD_ENTRY_EX(NETKVM_SHOW_PARAMS, (PFN_HANDLE_CMD)_NetKVMShowParamsCmdHandler,
                                                  CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL),
                              CREATE_CMD_ENTRY_EX(NETKVM_SHOW_PARAMINFO, (PFN_HANDLE_CMD)_NetKVMShowParamInfoCmdHandler,
                                                  CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL)
...
CMD_ENTRY g_TopLevelCommands[] = {
    CREATE_CMD_ENTRY_EX(NETKVM_RESTART_DEVICE, (PFN_HANDLE_CMD)_NetKVMRestartDeviceCmdHandler,
                        CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL),
    CREATE_CMD_ENTRY_EX(NETKVM_GET_PARAM, (PFN_HANDLE_CMD)_NetKVMGetParamCmdHandler, CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL),
    CREATE_CMD_ENTRY_EX(NETKVM_SET_PARAM, (PFN_HANDLE_CMD)_NetKVMSetParamCmdHandler,
                        CMD_FLAG_PRIVATE | CMD_FLAG_LOCAL)};
...
DWORD WINAPI _NetKVMDumpCdmHandler(__in PWCHAR pwszRouter, __in WCHAR ** /* ppwcArguments */,
                                   __in DWORD /* dwArgCount */, __in PVOID /* pvData */)
{
    try
    {
