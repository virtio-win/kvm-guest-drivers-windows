...
class CServiceManager
{
  public:
    ...
    bool Install(LPCTSTR name, LPCTSTR binary)
    {
        ...
        SC_HANDLE h =
            CreateService(m_Handle, name, name, SERVICE_STOP | SERVICE_START | SERVICE_QUERY_STATUS, ServiceType(),
                          SERVICE_DEMAND_START, SERVICE_ERROR_NORMAL, s, NULL, 0, NULL, NULL, NULL);
        if (h)
        {
...
class CService
{
  public:
    ...
    {
        LPCSTR names[] = {"Unknown", "Stopped",          "Start Pending", "Stop Pending",
        ...
        case SERVICE_CONTROL_STOP:
            ...
            else
            {
                Log("%s: got STOP, current state %s(%d)", __FUNCTION__,
                    CService::StateName(m_State.status.dwCurrentState), m_State.status.dwCurrentState);
            }
            break;
        default:
            break;
