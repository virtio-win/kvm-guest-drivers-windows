...
static NTSTATUS ioctl_request_mmap(const PDEVICE_CONTEXT DeviceContext,
                                   ...
                                   const size_t OutputBufferLength,
                                   const WDFREQUEST Request,
                                   size_t *BytesReturned, BOOLEAN ForKernel);
...
VOID IVSHMEMEvtIoDeviceControl(_In_ WDFQUEUE Queue, _In_ WDFREQUEST Request,
  ...
  // revision 0 devices have to wait until the shared memory has been provided
  // to the vm
  if (deviceContext->devRegisters->ivProvision < 0) {
    DEBUG_INFO("Device not ready yet, ivProvision = %d",
               deviceContext->devRegisters->ivProvision);
  ...
  case IOCTL_IVSHMEM_REQUEST_PEERID:
    status = ioctl_request_peerid(deviceContext, OutputBufferLength, Request,
                                  &bytesReturned);
    break;

  case IOCTL_IVSHMEM_REQUEST_SIZE:
    status = ioctl_request_size(deviceContext, OutputBufferLength, Request,
                                &bytesReturned);
    break;

  case IOCTL_IVSHMEM_REQUEST_MMAP:
    status =
        ioctl_request_mmap(deviceContext, InputBufferLength, OutputBufferLength,
                           Request, &bytesReturned, FALSE);
    break;
  case IOCTL_IVSHMEM_REQUEST_KMAP:
    status =
        ioctl_request_mmap(deviceContext, InputBufferLength, OutputBufferLength,
                           Request, &bytesReturned, TRUE);
    break;
  ...
  case IOCTL_IVSHMEM_RING_DOORBELL:
    status = ioctl_ring_doorbell(deviceContext, InputBufferLength, Request,
                                 &bytesReturned);
    break;

  case IOCTL_IVSHMEM_REGISTER_EVENT:
    status = ioctl_register_event(deviceContext, InputBufferLength, Request,
                                  &bytesReturned);
    break;
  }
...
VOID IVSHMEMEvtIoStop(_In_ WDFQUEUE Queue, _In_ WDFREQUEST Request,
...
VOID IVSHMEMEvtDeviceFileCreate(_In_ WDFDEVICE Device, _In_ WDFREQUEST Request,
  ...
  NTSTATUS status =
      WdfObjectAllocateContext(FileObject, &deviceAttributes, &fileContext);
  if (!NT_SUCCESS(status)) {
    DEBUG_ERROR(
        "EvtDeviceFileCreate callback: WdfObjectAllocateContext failed: %d",
        status);
...
VOID IVSHMEMEvtDeviceFileCleanup(_In_ WDFFILEOBJECT FileObject) {
  ...
  while (entry != &deviceContext->eventList) {
    ...
    PIVSHMEMEventListEntry event =
        CONTAINING_RECORD(entry, IVSHMEMEventListEntry, ListEntry);
...
static NTSTATUS ioctl_request_peerid(const PDEVICE_CONTEXT DeviceContext,
  ...
  if (OutputBufferLength != sizeof(IVSHMEM_PEERID)) {
    DEBUG_ERROR(
        "IOCTL_IVSHMEM_REQUEST_PEERID: Invalid size, expected %u but got %u",
        sizeof(IVSHMEM_PEERID), OutputBufferLength);
    return STATUS_INVALID_BUFFER_SIZE;
  }

  IVSHMEM_PEERID *out = NULL;
  if (!NT_SUCCESS(WdfRequestRetrieveOutputBuffer(Request, OutputBufferLength,
                                                 (PVOID *)&out, NULL))) {
    DEBUG_ERROR(
        "%s",
        "IOCTL_IVSHMEM_REQUEST_PEERID: Failed to retrieve the output buffer");
...
static NTSTATUS ioctl_request_size(const PDEVICE_CONTEXT DeviceContext,
  ...
  if (OutputBufferLength != sizeof(IVSHMEM_SIZE)) {
    DEBUG_ERROR(
        "IOCTL_IVSHMEM_REQUEST_SIZE: Invalid size, expected %u but got %u",
        sizeof(IVSHMEM_SIZE), OutputBufferLength);
    return STATUS_INVALID_BUFFER_SIZE;
  }
  ...
  if (!NT_SUCCESS(WdfRequestRetrieveOutputBuffer(Request, OutputBufferLength,
    ...
    DEBUG_ERROR(
        "%s",
        "IOCTL_IVSHMEM_REQUEST_SIZE: Failed to retrieve the output buffer");
    return STATUS_INVALID_USER_BUFFER;
  }
...
static NTSTATUS ioctl_request_mmap(const PDEVICE_CONTEXT DeviceContext,
  ...
  if (InputBufferLength != sizeof(IVSHMEM_MMAP_CONFIG)) {
    DEBUG_ERROR(
        "IOCTL_IVSHMEM_MMAP: Invalid input size, expected %u but got %u",
        sizeof(IVSHMEM_MMAP_CONFIG), InputBufferLength);
    return STATUS_INVALID_BUFFER_SIZE;
  }

  PIVSHMEM_MMAP_CONFIG in;
  if (!NT_SUCCESS(WdfRequestRetrieveInputBuffer(Request, InputBufferLength,
                                                (PVOID)&in, NULL))) {
    DEBUG_ERROR("%s",
                "IOCTL_IVSHMEM_MMAP: Failed to retrieve the input buffer");
    return STATUS_INVALID_USER_BUFFER;
  }
  ...
  const size_t bufferLen =
      is32Bit ? sizeof(IVSHMEM_MMAP32) : sizeof(IVSHMEM_MMAP);
#else
  ...
  if (OutputBufferLength != bufferLen) {
    DEBUG_ERROR(
        "IOCTL_IVSHMEM_REQUEST_MMAP: Invalid size, expected %u but got %u",
        bufferLen, OutputBufferLength);
    return STATUS_INVALID_BUFFER_SIZE;
  }

  if (!NT_SUCCESS(WdfRequestRetrieveOutputBuffer(Request, bufferLen,
                                                 (PVOID *)&buffer, NULL))) {
    DEBUG_ERROR(
        "%s",
        "IOCTL_IVSHMEM_REQUEST_MMAP: Failed to retrieve the output buffer");
    return STATUS_INVALID_USER_BUFFER;
  }

  __try {
    fileContext->shmemMap = MmMapLockedPagesSpecifyCache(
        DeviceContext->shmemMDL, ForKernel ? KernelMode : UserMode, cacheType,
        NULL, FALSE, NormalPagePriority | MdlMappingNoExecute);
  } __except (EXCEPTION_EXECUTE_HANDLER) {
    DEBUG_ERROR("%s",
                "IOCTL_IVSHMEM_REQUEST_MMAP: Exception trying to map pages");
...
static NTSTATUS ioctl_ring_doorbell(const PDEVICE_CONTEXT DeviceContext,
  ...
  if (InputBufferLength != sizeof(IVSHMEM_RING)) {
    DEBUG_ERROR(
        "IOCTL_IVSHMEM_RING_DOORBELL: Invalid size, expected %u but got %u",
        sizeof(IVSHMEM_RING), InputBufferLength);
    return STATUS_INVALID_BUFFER_SIZE;
  }
  ...
  if (!NT_SUCCESS(WdfRequestRetrieveInputBuffer(Request, InputBufferLength,
    ...
    DEBUG_ERROR(
        "%s",
        "IOCTL_IVSHMEM_RING_DOORBELL: Failed to retrieve the input buffer");
    return STATUS_INVALID_USER_BUFFER;
  }

  WRITE_REGISTER_ULONG(&DeviceContext->devRegisters->doorbell,
...
static NTSTATUS ioctl_register_event(const PDEVICE_CONTEXT DeviceContext,
  ...
  // ensure someone else other then the owner isn't attempting to register
  // events
  if (!fileContext->shmemMap) {
  ...
  if (InputBufferLength != sizeof(IVSHMEM_EVENT)) {
    DEBUG_ERROR(
        "IOCTL_IVSHMEM_REGISTER_EVENT: Invalid size, expected %u but got %u",
        sizeof(PIVSHMEM_EVENT), InputBufferLength);
    return STATUS_INVALID_BUFFER_SIZE;
  }
  ...
  if (!NT_SUCCESS(WdfRequestRetrieveInputBuffer(Request, InputBufferLength,
    ...
    DEBUG_ERROR(
        "%s",
        "IOCTL_IVSHMEM_REGISTER_EVENT: Failed to retrieve the input buffer");
    return STATUS_INVALID_USER_BUFFER;
  }

  PRKEVENT hObject;
  if (!NT_SUCCESS(ObReferenceObjectByHandle(
          in->event, SYNCHRONIZE | EVENT_MODIFY_STATE, *ExEventObjectType,
          UserMode, &hObject, NULL))) {
  ...
  KeAcquireSpinLock(&DeviceContext->eventListLock, &oldIRQL);
  {
    ...
    if (!done) {
      DEBUG_ERROR(
          "IOCTL_IVSHMEM_REGISTER_EVENT: deviceContext->eventBufferUsed (%u) < "
          "MAX_EVENTS (%u) but no slots found!",
