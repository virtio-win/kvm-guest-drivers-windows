...
int __cdecl main() {
  ...
  while (1) {
    ...
    devHandle =
        CreateFile(infData->DevicePath, 0, 0, NULL, OPEN_EXISTING, 0, 0);
    ...
    if (!DeviceIoControl(devHandle, IOCTL_IVSHMEM_REQUEST_PEERID, NULL, 0,
                         &peer, sizeof(IVSHMEM_PEERID), &ulReturnedLength,
                         NULL)) {
    ...
    IVSHMEM_SIZE size = 0;
    if (!DeviceIoControl(devHandle, IOCTL_IVSHMEM_REQUEST_SIZE, NULL, 0, &size,
                         sizeof(IVSHMEM_SIZE), &ulReturnedLength, NULL)) {
      TEST_FAIL("DeviceIoControl");
      break;
    }
    ...
    if (!DeviceIoControl(devHandle, IOCTL_IVSHMEM_REQUEST_MMAP, &config,
                         sizeof(IVSHMEM_MMAP_CONFIG), &map,
                         sizeof(IVSHMEM_MMAP), &ulReturnedLength, NULL)) {
      TEST_FAIL("DeviceIoControl");
      break;
    }
    ...
    if (DeviceIoControl(devHandle, IOCTL_IVSHMEM_REQUEST_MMAP, &config,
                        sizeof(IVSHMEM_MMAP_CONFIG), &map, sizeof(IVSHMEM_MMAP),
                        &ulReturnedLength, NULL)) {
    ...
    if (!DeviceIoControl(devHandle2, IOCTL_IVSHMEM_REQUEST_MMAP, &config,
                         sizeof(IVSHMEM_MMAP_CONFIG), &map,
                         sizeof(IVSHMEM_MMAP), &ulReturnedLength, NULL)) {
    ...
    ring.peerID = 0;
    ring.vector = 0;
    if (!DeviceIoControl(devHandle, IOCTL_IVSHMEM_RING_DOORBELL, &ring,
                         sizeof(IVSHMEM_RING), NULL, 0, &ulReturnedLength,
                         NULL)) {
      TEST_FAIL("DeviceIoControl");
      break;
    }
    ...
    if (!DeviceIoControl(devHandle, IOCTL_IVSHMEM_RELEASE_MMAP, NULL, 0, NULL,
                         0, &ulReturnedLength, NULL)) {
      TEST_FAIL("DeviceIoControl");
      break;
    }
    TEST_PASS();
    ...
    if (map.vectors > 0) {
      ...
      if (!DeviceIoControl(devHandle, IOCTL_IVSHMEM_REGISTER_EVENT, &event,
                           sizeof(IVSHMEM_EVENT), NULL, 0, &ulReturnedLength,
                           NULL)) {
