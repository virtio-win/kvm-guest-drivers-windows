...
DEFINE_GUID(GUID_DEVINTERFACE_IVSHMEM, 0xdf576976, 0x569d, 0x4672, 0x95, 0xa0,
            0xf5, 0x7e, 0x4e, 0xa0, 0xb2, 0x10);
...
typedef struct IVSHMEM_MMAP_CONFIG {
  UINT8 cacheMode; // the caching mode of the mapping, see IVSHMEM_CACHE_* for
                   // options
...
typedef struct IVSHMEM_EVENT {
  ...
  BOOLEAN singleShot; // set to TRUE if you want the driver to only trigger this
                      // event once
...
#define IOCTL_IVSHMEM_REQUEST_KMAP                                             \
