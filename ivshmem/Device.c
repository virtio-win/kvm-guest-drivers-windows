...
NTSTATUS IVSHMEMCreateDevice(_Inout_ PWDFDEVICE_INIT DeviceInit) {
  ...
  WDF_FILEOBJECT_CONFIG_INIT(&fileConfig, IVSHMEMEvtDeviceFileCreate, NULL,
                             IVSHMEMEvtDeviceFileCleanup);
  ...
  status =
      WdfDeviceCreateDeviceInterface(device, &GUID_DEVINTERFACE_IVSHMEM, NULL);
...
PVOID IVSHMEMMmMapIoSpace(_In_ PHYSICAL_ADDRESS PhysicalAddress,
  ...
  if (pMmMapIoSpaceEx != NULL) {
    ...
    return pMmMapIoSpaceEx(PhysicalAddress, NumberOfBytes,
...
NTSTATUS
IVSHMEMEvtDevicePrepareHardware(_In_ WDFDEVICE Device,
  ...
  if (deviceContext->interruptCount > 0) {
    deviceContext->interrupts = (WDFINTERRUPT *)ExAllocatePoolUninitialized(
        IVSHMEM_NONPAGED_POOL,
        sizeof(WDFINTERRUPT) * deviceContext->interruptCount, 'sQRI');

    if (!deviceContext->interrupts) {
      DEBUG_ERROR("Failed to allocate space for %d interrupts",
  ...
  for (ULONG i = 0; i < resCount; ++i) {
    ...
    if (descriptor->Type == CmResourceTypeMemory) {
      ...
      if (memIndex == 0) {
        ...
        deviceContext->devRegisters =
            (PIVSHMEMDeviceRegisters)IVSHMEMMmMapIoSpace(
        ...
        if ((deviceContext->interruptCount == 0 && memIndex == 1) ||
            memIndex == 2) {
          ...
          DEBUG_INFO("memIndex = %d pa = %llx (%llx) size = %lx (%lx)",
                     memIndex, descriptor->u.Memory.Start.QuadPart,
                     deviceContext->shmemAddr.PhysicalAddress.QuadPart,
                     descriptor->u.Memory.Length,
                     deviceContext->shmemAddr.NumberOfBytes);

#if (NTDDI_VERSION >= NTDDI_WIN8)
          result = MmAllocateMdlForIoSpace(&deviceContext->shmemAddr, 1,
                                           &deviceContext->shmemMDL);
#else
          deviceContext->shmemAddr.VirtualAddress =
              MmMapIoSpace(deviceContext->shmemAddr.PhysicalAddress,
                           deviceContext->shmemAddr.NumberOfBytes, MmNonCached);
          if (deviceContext->shmemAddr.VirtualAddress) {
            deviceContext->shmemMDL =
                IoAllocateMdl(deviceContext->shmemAddr.VirtualAddress,
      ...
      DEBUG_INFO("memIndex = %d va = %p mdl = %p", memIndex,
                 deviceContext->shmemAddr.VirtualAddress,
                 deviceContext->shmemMDL);
      ++memIndex;
      continue;
    ...
    if (descriptor->Type == CmResourceTypeInterrupt &&
      ...
      WDF_INTERRUPT_CONFIG_INIT(&irqConfig, IVSHMEMInterruptISR,
                                IVSHMEMInterruptDPC);
#if (NTDDI_VERSION >= NTDDI_WIN8)
      ...
      if (++deviceContext->interruptsUsed == 65)
        DEBUG_INFO("%s", "This driver does not support > 64 interrupts, they "
                         "will be ignored in the ISR.");
...
NTSTATUS
IVSHMEMEvtDeviceReleaseHardware(_In_ WDFDEVICE Device,
                                _In_ WDFCMRESLIST ResourcesTranslated) {
  ...
  if (deviceContext->devRegisters) {
    MmUnmapIoSpace(deviceContext->devRegisters,
                   sizeof(PIVSHMEMDeviceRegisters));
  }
...
NTSTATUS IVSHMEMEvtD0Entry(_In_ WDFDEVICE Device,
...
NTSTATUS IVSHMEMEvtD0Exit(_In_ WDFDEVICE Device,
                          _In_ WDF_POWER_DEVICE_STATE PreviousState) {
  UNREFERENCED_PARAMETER(Device);
  UNREFERENCED_PARAMETER(PreviousState);
  PAGED_CODE();
...
void IVSHMEMInterruptDPC(_In_ WDFINTERRUPT Interrupt,
