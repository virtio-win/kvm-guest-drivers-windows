...
CDevice::CDevice () { m_hDevice = INVALID_HANDLE_VALUE; }
...
BOOL
CDevice::Init (BOOL ovrl, UINT index)
{
  PWCHAR DevicePath = NULL;
  if ((DevicePath = GetDevicePath (index, (LPGUID)&GUID_VIOSERIAL_PORT))
      != NULL)
    {
      m_hDevice = CreateFile (
          DevicePath, GENERIC_WRITE | GENERIC_READ, 0, NULL, OPEN_EXISTING,
          ovrl ? FILE_FLAG_OVERLAPPED : FILE_ATTRIBUTE_NORMAL, NULL);
...
BOOL
CDevice::Write (PVOID buf, size_t *size)
{
  BOOL res = FALSE;
  ULONG ret = 0;
  DWORD bytes = *size;
  ...
  if (!res)
    ...
    {
      printf (
          "Write vioserial device error. written = 0x%x, expected = 0x%x\n",
          ret, bytes);
      *size = ret;
      ret = FALSE;
    }
...
BOOL
CDevice::WriteEx (PVOID buf, size_t *size)
{
  BOOL res = FALSE;
  ULONG ret = 0;
  DWORD bytes = *size;
...
BOOL
CDevice::Read (PVOID buf, size_t *size)
{
  BOOL res = FALSE;
  DWORD ret;
  DWORD bytes = *size;
...
BOOL
CDevice::ReadEx (PVOID buf, size_t *size)
{
  BOOL res = FALSE;
  DWORD ret;
  DWORD bytes = *size;
...
BOOL
CDevice::GetInfo (PVOID buf, size_t *size)
{
  BOOL res = FALSE;
  ...
  res = DeviceIoControl (m_hDevice, IOCTL_GET_INFORMATION, NULL, 0, buf,
                         ulOutLength, &ulReturnedLength, NULL);
...
PTCHAR
CDevice::GetDevicePath (UINT index, IN LPGUID InterfaceGuid)
{
  ...
  bResult = SetupDiEnumDeviceInterfaces (HardwareDeviceInfo, 0, InterfaceGuid,
  ...
  DeviceInterfaceDetailData = (PSP_DEVICE_INTERFACE_DETAIL_DATA)LocalAlloc (
