...
BOOL GetInfoTest(__in CDevice *pDev)
{
    PVOID buf = NULL;
...
BOOL WriteTest(__in CDevice *pDev, __in BOOLEAN ovrl)
{
    PUCHAR buf = NULL;
    BOOLEAN res = TRUE;
    int i;
...
BOOL WriteTestCycl(__in CDevice *pDev, __in BOOLEAN ovrl)
{
    int ch;
    for (;;)
    {
        if (!WriteTest(pDev, ovrl))
        {
            return FALSE;
        }
        ch = getchar();
...
BOOL ReadTest(__in CDevice *pDev, __in BOOLEAN ovrl)
{
    PUCHAR buf = NULL;
    BOOLEAN res = TRUE;
...
BOOL ReadTestCycl(__in CDevice *pDev, __in BOOLEAN ovrl)
{
    int ch;
    for (;;)
    {
        if (!ReadTest(pDev, ovrl))
        {
            return FALSE;
        }
        if (_kbhit())
