...
VIOSerialAllocateSinglePageBuffer (IN VirtIODevice *vdev, IN ULONG id)
{
  ...
  buf = ExAllocatePoolUninitialized (NonPagedPool, sizeof (PORT_BUFFER),
...
size_t
VIOSerialSendBuffers (IN PVIOSERIAL_PORT Port, IN PWRITE_BUFFER_ENTRY Entry)
{
...
DmaWriteCallback (PVIRTIO_DMA_TRANSACTION_PARAMS params)
{
  ...
  ret = virtqueue_add_buf (vq, sg, params->sgList->NumberOfElements, 0, Entry,
                           NULL, 0);
...
VOID
VIOSerialFreeBuffer (IN PPORT_BUFFER buf)
{
  ASSERT (buf);
...
VOID
VIOSerialProcessInputBuffers (IN PVIOSERIAL_PORT Port)
{
  NTSTATUS status;
  ULONG Read = 0;
      ...
      if (status != STATUS_CANCELLED)
        {
          ...
          status = WdfRequestRetrieveOutputBuffer (Port->PendingReadRequest, 0,
                                                   &Buffer, &Length);
          if (NT_SUCCESS (status))
            ...
            {
              TraceEvents (TRACE_LEVEL_ERROR, DBG_QUEUEING,
                           "Failed to retrieve output buffer (Status: %x "
                           "Request: %p).\n",
                           status, Request);
            }
        }
      ...
      while ((buffer = virtqueue_get_buf (vq, &len)) != NULL)
        {
          ...
          while (iter->Next != NULL)
            {
              PWRITE_BUFFER_ENTRY entry = CONTAINING_RECORD (
                  iter->Next, WRITE_BUFFER_ENTRY, ListEntry);
      ...
      if (request != NULL)
        {
          WdfRequestCompleteWithInformation (
              request, STATUS_SUCCESS, WdfRequestGetInformation (request));
        }
      WdfObjectDelete (entry->EntryHandle);
    };
