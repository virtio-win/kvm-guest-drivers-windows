  ...
  for (;;)
    {
      ...
      WDF_CHILD_IDENTIFICATION_DESCRIPTION_HEADER_INIT (&port.Header,
                                                        sizeof (port));
      WDF_CHILD_RETRIEVE_INFO_INIT (&childInfo, &port.Header);

      status = WdfChildListRetrieveNextDevice (list, &iterator, &hChild,
                                               &childInfo);
...
VOID
VIOSerialAddPort (IN WDFDEVICE Device, IN ULONG id)
{
  VIOSERIAL_PORT port;
  ...
  status = WdfChildListAddOrUpdateChildDescriptionAsPresent (
      WdfFdoGetDefaultChildList (Device), &port.Header, NULL);

  if (status == STATUS_OBJECT_NAME_EXISTS)
    {
      TraceEvents (TRACE_LEVEL_INFORMATION, DBG_PNP,
                   "The description is already present in the list, the "
...
VOID
VIOSerialRemovePort (IN WDFDEVICE Device, IN PVIOSERIAL_PORT Port)
{
  ...
  status
      = WdfChildListUpdateChildDescriptionAsMissing (PortList, &Port->Header);
...
VOID
VIOSerialInitPortConsoleWork (IN WDFWORKITEM WorkItem)
{
  ...
  VIOSerialSendCtrlMsg (pport->BusDevice, pport->PortId,
...
VOID
VIOSerialInitPortConsole (IN WDFDEVICE WdfDevice, IN PVIOSERIAL_PORT port)
{
...
VOID
VIOSerialDiscardPortDataLocked (IN PVIOSERIAL_PORT port)
{
  struct virtqueue *vq;
...
VIOSerialDeviceListCreatePdo (IN WDFCHILDLIST DeviceList,
                              IN PWDF_CHILD_IDENTIFICATION_DESCRIPTION_HEADER
                              ...
                              IN PWDFDEVICE_INIT ChildInit)
{
  ...
  pport
      = CONTAINING_RECORD (IdentificationDescription, VIOSERIAL_PORT, Header);
  ...
  do
    {
      ...
      status
          = WdfPdoInitAssignRawDevice (ChildInit, &GUID_DEVCLASS_PORT_DEVICE);
      if (!NT_SUCCESS (status))
        {
          TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
                       "WdfPdoInitAssignRawDevice failed - 0x%x\n", status);
          break;
        }
      ...
      if (!NT_SUCCESS (status))
        {
          TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
                       "WdfPdoInitAssignDeviceID failed - 0x%x\n", status);
          break;
        }
      ...
      if (!NT_SUCCESS (status))
        {
          TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
                       "WdfPdoInitAddHardwareID failed - 0x%x\n", status);
          break;
        }
      ...
      if (!NT_SUCCESS (status))
        {
          TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
                       "WdfPdoInitAssignInstanceID failed - 0x%x\n", status);
          break;
        }

      status = RtlUnicodeStringPrintf (&buffer, L"vport%up%u", pport->DeviceId,
                                       pport->PortId);
      if (!NT_SUCCESS (status))
        {
          TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
      ...
      status = WdfPdoInitAddDeviceText (ChildInit, &buffer, &deviceLocation,
                                        0x409);
      if (!NT_SUCCESS (status))
        {
          TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
      ...
      WDF_FILEOBJECT_CONFIG_INIT (&fileConfig, VIOSerialPortCreate,
                                  VIOSerialPortClose, WDF_NO_EVENT_CALLBACK);

      WdfDeviceInitSetFileObjectConfig (ChildInit, &fileConfig,
      ...
      status = WdfIoQueueCreate (hChild, &queueConfig,
                                 WDF_NO_OBJECT_ATTRIBUTES, &pport->IoctlQueue);
      if (!NT_SUCCESS (status))
        {
          TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
      ...
      status = WdfDeviceConfigureRequestDispatching (
          hChild, pport->IoctlQueue, WdfRequestTypeDeviceControl);

      if (!NT_SUCCESS (status))
        {
          TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
                       "DeviceConfigureRequestDispatching failed (IoCtrl "
                       "Queue): 0x%x\n",
                       status);
          break;
        }
      ...
      status = WdfIoQueueCreate (hChild, &queueConfig,
                                 WDF_NO_OBJECT_ATTRIBUTES, &pport->ReadQueue);
      if (!NT_SUCCESS (status))
        {
          TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
      ...
      status = WdfDeviceConfigureRequestDispatching (hChild, pport->ReadQueue,
                                                     WdfRequestTypeRead);

      if (!NT_SUCCESS (status))
        {
          TraceEvents (
              TRACE_LEVEL_ERROR, DBG_PNP,
      ...
      status = WdfDeviceConfigureRequestDispatching (hChild, pport->WriteQueue,
                                                     WdfRequestTypeWrite);
      ...
      status = WdfDeviceCreateDeviceInterface (hChild, &GUID_VIOSERIAL_PORT,
                                               NULL);
  ...
  if (!NT_SUCCESS (status))
    {
      // We can send this before PDO is PRESENT since the device won't send any
      // response.
      VIOSerialSendCtrlMsg (pport->BusDevice, pport->PortId,
                            VIRTIO_CONSOLE_PORT_READY, 0, TRUE);
    }
...
VOID
VIOSerialPortRead (IN WDFQUEUE Queue, IN WDFREQUEST Request, IN size_t Length)
{
...
VOID
VIOSerialPortWrite (IN WDFQUEUE Queue, IN WDFREQUEST Request, IN size_t Length)
{
  NTSTATUS status;
  PVOID InBuf;
  ...
  status = WdfMemoryCreateFromLookaside (Context->WriteBufferLookaside,
                                         &EntryHandle);
  if (!NT_SUCCESS (status))
    {
      TraceEvents (TRACE_LEVEL_ERROR, DBG_WRITE,
  ...
  if (VIOSerialSendBuffers (Port, entry) == 0)
    {
      ...
      if (entry->dmaTransaction)
        {
          VirtIOWdfDeviceDmaTxComplete (GetOutQueue (Port)->vdev,
                                        entry->dmaTransaction);
        }
...
VOID
VIOSerialPortReadRequestCancel (IN WDFREQUEST Request)
{
...
VOID
VIOSerialPortWriteRequestCancel (IN WDFREQUEST Request)
{
  PVIOSERIAL_PORT Port
      = RawPdoSerialPortGetData (
            WdfIoQueueGetDevice (WdfRequestGetIoQueue (Request)))
            ->port;
  ...
  while ((iter = iter->Next) != NULL)
    {
      PWRITE_BUFFER_ENTRY entry
          = CONTAINING_RECORD (iter, WRITE_BUFFER_ENTRY, ListEntry);
...
VOID
VIOSerialPortDeviceControl (IN WDFQUEUE Queue, IN WDFREQUEST Request,
    ...
    case IOCTL_GET_INFORMATION_BUFFERED:
      {
        status = WdfRequestRetrieveOutputBuffer (
            Request, sizeof (VIRTIO_PORT_INFO), (PVOID *)&pport_info, &length);
        if (!NT_SUCCESS (status))
          {
        ...
        if (pdoData->port->NameString.Buffer)
          {
            ...
            if (length < sizeof (VIRTIO_PORT_INFO) + name_size)
              {
                // STATUS_BUFFER_OVERFLOW is not safe to use with buffered
                // IOCTL
            ...
            else
              {
                ...
                status = RtlStringCbCopyA (pport_info->Name, name_size - 1,
                                           pdoData->port->NameString.Buffer);
                if (!NT_SUCCESS (status))
                  {
                    TraceEvents (TRACE_LEVEL_ERROR, DBG_IOCTLS,
...
VOID
VIOSerialPortCreate (IN WDFDEVICE WdfDevice, IN WDFREQUEST Request,
...
VOID
VIOSerialPortClose (IN WDFFILEOBJECT FileObject)
{
  PRAWPDO_VIOSERIAL_PORT pdoData
      = RawPdoSerialPortGetData (WdfFileObjectGetDevice (FileObject));
      ...
      if (pdoData->port->GuestConnected)
        {
          VIOSerialSendCtrlMsg (pdoData->port->BusDevice,
                                pdoData->port->PortId,
                                VIRTIO_CONSOLE_PORT_OPEN, 0, TRUE);
        }
...
VOID
VIOSerialPortCreateName (IN WDFDEVICE WdfDevice, IN PVIOSERIAL_PORT port,
                         IN PPORT_BUFFER buf)
{
  WDF_OBJECT_ATTRIBUTES attributes;
  WDF_WORKITEM_CONFIG workitemConfig;
  ...
  if (port && !port->NameString.Buffer)
    ...
    {
      TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
                   "VIRTIO_CONSOLE_PORT_NAME invalid id = %d\n", cpkt->id);
    }
  TraceEvents (TRACE_LEVEL_INFORMATION, DBG_CREATE_CLOSE, "<-- %s\n",
               __FUNCTION__);
}
...
VOID
VIOSerialPortPnpNotify (IN WDFDEVICE WdfDevice, IN PVIOSERIAL_PORT port,
...
VOID
VIOSerialPortSymbolicNameWork (IN WDFWORKITEM WorkItem)
{
      ...
      if (pport->NameString.Buffer)
        {
          ...
          TraceEvents (TRACE_LEVEL_INFORMATION, DBG_PNP,
                       "deviceUnicodeString = %ws\n",
                       deviceUnicodeString.Buffer);

          status = RtlUnicodeStringPrintf (&symbolicLinkName, L"%ws%ws",
                                           L"\\DosDevices\\",
                                           deviceUnicodeString.Buffer);
          if (!NT_SUCCESS (status))
            {
          ...
          status
              = WdfDeviceCreateSymbolicLink (pport->Device, &symbolicLinkName);
          if (!NT_SUCCESS (status))
            {
              /* It is safe to print the buffer as follows as
               * RtlUnicodeStringPrints inserts null terminated character */
              TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
...
VOID
VIOSerialPortPnpNotifyWork (IN WDFWORKITEM WorkItem)
{
  PRAWPDO_VIOSERIAL_PORT pdoData = RawPdoSerialPortGetData (WorkItem);
  PVIOSERIAL_PORT pport = pdoData->port;
  ...
  status = RtlULongAdd (
      ...
      sizeof (VIRTIO_PORT_STATUS_CHANGE), &requiredSize);

  if (NT_SUCCESS (status))
    {
      notification
          = (PTARGET_DEVICE_CUSTOM_NOTIFICATION)ExAllocatePoolUninitialized (
          ...
          if (WdfDeviceGetDevicePnpState (pport->Device)
              == WdfDevStatePnpStarted)
            {
              status = IoReportTargetDeviceChangeAsynchronous (
                  WdfDeviceWdmGetPhysicalDevice (pport->Device), notification,
                  NULL, NULL);
              if (!NT_SUCCESS (status))
                {
                  TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP,
        ...
        DestinationIdentificationDescription)
{
  ...
  src = CONTAINING_RECORD (SourceIdentificationDescription, VIOSERIAL_PORT,
                           Header);
  dst = CONTAINING_RECORD (DestinationIdentificationDescription,
                           VIOSERIAL_PORT, Header);
  ...
  if (dst->NameString.Length)
    {
      ...
      dst->NameString.Buffer = (PCHAR)ExAllocatePoolUninitialized (
          NonPagedPool, dst->NameString.MaximumLength,
          VIOSERIAL_DRIVER_MEMORY_TAG);
      ...
      RtlCopyMemory (dst->NameString.Buffer, src->NameString.Buffer,
        ...
        SecondIdentificationDescription)
{
  ...
  lhs = CONTAINING_RECORD (FirstIdentificationDescription, VIOSERIAL_PORT,
                           Header);
  rhs = CONTAINING_RECORD (SecondIdentificationDescription, VIOSERIAL_PORT,
                           Header);
    ...
    PWDF_CHILD_IDENTIFICATION_DESCRIPTION_HEADER IdentificationDescription)
{
  ...
  // only for code analyzer; IdentificationDescription erroneously defined as
  // "out"
  IdentificationDescription->IdentificationDescriptionSize = sizeof (*pDesc);

  if (pDesc->NameString.Buffer)
    {
      ExFreePoolWithTag (pDesc->NameString.Buffer,
                         VIOSERIAL_DRIVER_MEMORY_TAG);
...
VOID
VIOSerialPortReadIoStop (IN WDFQUEUE Queue, IN WDFREQUEST Request,
...
VOID
VIOSerialPortWriteIoStop (IN WDFQUEUE Queue, IN WDFREQUEST Request,
                          IN ULONG ActionFlags)
{
  PRAWPDO_VIOSERIAL_PORT pdoData
      = RawPdoSerialPortGetData (WdfIoQueueGetDevice (Queue));
...
NTSTATUS
VIOSerialPortEvtDeviceD0Entry (IN WDFDEVICE Device,
...
NTSTATUS
VIOSerialPortEvtDeviceD0Exit (IN WDFDEVICE Device,
                              IN WDF_POWER_DEVICE_STATE TargetState)
{
  ...
  VirtIOWdfDeviceFreeDmaMemoryByTag (GetInQueue (Port)->vdev,
                                     Port->DmaGroupTag);
