...
class PnPControl
{
  ...
  PnPControl ()
      : Thread (INVALID_HANDLE_VALUE), Notification (0, 0, 0),
...
public:
  ...
  VOID RegisterNotification (PVOID port, VIOSERIALNOTIFYCALLBACK pfn,
                             PVOID ptr);
  const PnPNotification &
  GetNotification () const
  {
    return Notification;
  }
  void
  DispatchPnpMessage (const PnPNotification &notification)
  {
...
private:
  static void ProcessPnPNotification (PnPControl *ptr,
                                      PnPNotification newNotification);
...
public:
  ...
  virtual ~SerialController () { printf ("~SerialController.\n"); }
  virtual void
  handleEvent (const PnPControl &ref)
  {
