...
SerialPort::SerialPort(wstring LinkName, PnPControl *ptr)
{
    ...
    BOOL res = DeviceIoControl(Handle, IOCTL_GET_INFORMATION, NULL, 0, buf, sizeof(buf), &ulReturnedLength, NULL);
    if (res == FALSE)
    {
...
{
    Handle = CreateFile(Name.c_str(), GENERIC_WRITE | GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
    if (Handle == INVALID_HANDLE_VALUE)
    {
        return FALSE;
    }
