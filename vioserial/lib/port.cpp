...
SerialPort::SerialPort (wstring LinkName, PnPControl *ptr)
{
  ...
  Reference = 0;
  Handle = CreateFile (Name.c_str (), GENERIC_WRITE | GENERIC_READ, 0, NULL,
  ...
  BOOL res = DeviceIoControl (Handle, IOCTL_GET_INFORMATION, NULL, 0, buf,
                              sizeof (buf), &ulReturnedLength, NULL);
  if (res == FALSE)
    {
...
BOOL
SerialPort::ReadPort (PVOID buf, size_t *len)
{
  BOOL res = FALSE;
  DWORD ret;
  ...
  res = ReadFile (Handle, buf, bytes, &ret, &ol);
  if (!res)
    {
...
BOOL
SerialPort::WritePort (PVOID buf, size_t *len)
{
  BOOL res = FALSE;
  ULONG ret = 0;
  DWORD bytes = (DWORD)(*len);
  ...
  res = WriteFile (Handle, buf, bytes, &ret, &ol);
  if (!res)
    {
      DWORD err = GetLastError ();
      if (err != ERROR_IO_PENDING)
...
void
SerialPort::handleEvent (const PnPControl &ref)
{
    ...
    case DBT_CUSTOMEVENT:
      {
        ...
        if (IsEqualGUID (GUID_VIOSERIAL_PORT_CHANGE_STATUS,
                         pHdr->dbch_eventguid)
            && (pHdr->dbch_handle == Handle))
          {
            PVIRTIO_PORT_STATUS_CHANGE pEventInfo
                = (PVIRTIO_PORT_STATUS_CHANGE)pHdr->dbch_data;
