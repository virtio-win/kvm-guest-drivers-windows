...
BOOL
RunBenchmarkWorker (HANDLE hPort, SIZE_T cbRequestSize, DWORD dwConcurrency,
                    DWORD dwIterations)
{
  ...
  while (!_kbhit () && dwIterations > 0)
    {
          ...
          if (lpOverlapped[i].hEvent == NULL)
            {
              ...
              if (!IOPROVIDER::StartIO (hPort, lpBuffer.get (),
                                        (DWORD)cbRequestSize,
                                        &lpOverlapped[i]))
                {
      ...
      // wait for any of the overlapped events to get signaled or for a timer
      // tick
      DWORD dwWaitRes = WaitForMultipleObjects (
          dwConcurrency + 1, lpHandles.get (), FALSE, INFINITE);
          ...
          if (cbTotalTransferred != 0)
            {
              ...
              dwIterations--;
               ...
               && dwWaitRes <= WAIT_OBJECT_0 + dwConcurrency)
        {
          // one of the overlapped events was signaled
          for (DWORD idx = dwWaitRes - WAIT_OBJECT_0 - 1; idx < dwWaitRes;
               idx++)
            {
              if (WaitForSingleObject (lpEvents[idx], 0) == WAIT_OBJECT_0)
                {
                  ...
                  DWORD cbTransferred;
                  if (IOPROVIDER::CompleteIO (hPort, (DWORD)cbRequestSize,
                                              &lpOverlapped[idx],
                                              &cbTransferred))
                    {
  ...
  for (DWORD i = 0; i < dwConcurrency; i++)
    {
      if (lpOverlapped[i].hEvent != NULL)
        {
          if (CancelIoEx (hPort, &lpOverlapped[i])
              || GetLastError () != ERROR_NOT_FOUND)
            {
              DWORD cbTransferred;
              GetOverlappedResult (hPort, &lpOverlapped[i], &cbTransferred,
                                   TRUE);
            }
        }
    }
...
BOOL
RunWriteBenchmark (HANDLE hPort, SIZE_T cbRequestSize, DWORD dwConcurrency,
                   DWORD dwIterations)
{
  ...
  public:
    static BOOL
    StartIO (HANDLE handle, LPVOID lpBuffer, DWORD cbBuffer,
    ...
    static BOOL
    CompleteIO (HANDLE handle, DWORD cbBuffer, LPOVERLAPPED lpOverlapped,
                LPDWORD lpcbTransferred)
    {
      ...
      if (GetOverlappedResult (handle, lpOverlapped, lpcbTransferred, FALSE))
        {
          if (*lpcbTransferred != cbBuffer)
            {
              wprintf (
                  L"Written %d bytes which is not equal to request size %d",
                  *lpcbTransferred, cbBuffer);
            }
          return TRUE;
        }
      return FALSE;
  ...
  wprintf (L"Writing %Iu byte buffers, %u parallel requests, for %u seconds\n",
           cbRequestSize, dwConcurrency, dwIterations);
  return RunBenchmarkWorker<WriteIOProvider> (hPort, cbRequestSize,
                                              dwConcurrency, dwIterations);
}

BOOL
RunReadBenchmark (HANDLE hPort, SIZE_T cbRequestSize, DWORD dwConcurrency,
                  DWORD dwIterations)
{
  ...
  public:
    static BOOL
    StartIO (HANDLE hPort, LPVOID lpBuffer, DWORD cbBuffer,
             LPOVERLAPPED lpOverlapped)
    {
    ...
    static BOOL
    CompleteIO (HANDLE hPort, DWORD cbBuffer, LPOVERLAPPED lpOverlapped,
                LPDWORD lpcbTransferred)
    {
  ...
  wprintf (
      ...
      cbRequestSize, dwConcurrency, dwIterations);
  return RunBenchmarkWorker<ReadIOProvider> (hPort, cbRequestSize,
                                             dwConcurrency, dwIterations);
}

BOOL
RunBenchmark (LPCWSTR wszPortName, BenchmarkType type, SIZE_T cbRequestSize,
              DWORD dwConcurrency, DWORD dwIterations)
{
  WCHAR wszNameBuffer[MAX_PATH];
  swprintf (wszNameBuffer, _countof (wszNameBuffer), L"\\\\.\\%s",
            wszPortName);

  HANDLE hPort = CreateFile (wszNameBuffer, GENERIC_WRITE | GENERIC_READ, 0,
  ...
  do
    {
      switch (type)
        {
        case ReadBenchmark:
          bResult = RunReadBenchmark (hPort, cbRequestSize, dwConcurrency,
                                      dwIterations);
          break;
        case WriteBenchmark:
          bResult = RunWriteBenchmark (hPort, cbRequestSize, dwConcurrency,
                                       dwIterations);
          break;
        default:
