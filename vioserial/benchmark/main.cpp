...
{
  wprintf (L"USAGE:\n");
  wprintf (L"benchmark <type> <port_name> [-s <request_size>] [-c "
           L"<concurrency>] [-t <time>]\n");
  wprintf (L"\n");
  wprintf (L"<type>           (r)ead or (w)rite\n");
  wprintf (L"<port_name>      name of the port to use\n");
  wprintf (
      L"<request_size>   size of each I/O request in bytes, %u by default\n",
      DEFAULT_REQUEST_SIZE);
  wprintf (
      L"<concurrency>    number of requests to run in parallel, if omitted\n");
  wprintf (L"                 benchmarks all concurrency levels 1 and up\n");
  wprintf (L"<time>           time in seconds to run for at each concurrency "
           L"level,\n");
  wprintf (L"                 %u by default\n", DEFAULT_NUM_OF_ITERATIONS);
  wprintf (L"\n");
  wprintf (
      L"Example: benchmark w com.redhat.rhevm.vdsm1 -s 8192 -c 2 -t 10\n");
}

template <typename T>
BOOL
ParseUInt (LPCWSTR wszStr, T *lpRes)
{
  LPWCH lpEnd;
  ...
  for (ULONG i = 3; i < argc; i += 2)
    {
      ...
      if ((wszArg[0] == '-' || wszArg[0] == '/') && wszArg[1] != 0
          && wszArg[2] == 0)
        {
          BOOL bSuccess = FALSE;
          switch (wszArg[1])
